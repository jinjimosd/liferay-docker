...
function build_docker_image {
	...
	do
		if [[ ${LIFERAY_DOCKER_RELEASE_FILE_URL%} == */snapshot-* ]]
		then
			DOCKER_IMAGE_TAGS+=("${LIFERAY_DOCKER_REPOSITORY}jinjimosd/${DOCKER_IMAGE_NAME}:${release_branch}-${release_version_single}-${release_hash}")
			DOCKER_IMAGE_TAGS+=("${LIFERAY_DOCKER_REPOSITORY}jinjimosd/${DOCKER_IMAGE_NAME}:${release_branch}-$(date "${CURRENT_DATE}" "+%Y%m%d")")
			DOCKER_IMAGE_TAGS+=("${LIFERAY_DOCKER_REPOSITORY}jinjimosd/${DOCKER_IMAGE_NAME}:${release_branch}")
		else
			DOCKER_IMAGE_TAGS+=("${LIFERAY_DOCKER_REPOSITORY}jinjimosd/${DOCKER_IMAGE_NAME}:${release_version_single}-d$(./release_notes.sh get-version)-${TIMESTAMP}")
			DOCKER_IMAGE_TAGS+=("${LIFERAY_DOCKER_REPOSITORY}jinjimosd/${DOCKER_IMAGE_NAME}:${release_version_single}")
		fi
	done

	if [[ "${LIFERAY_DOCKER_LATEST}" = "true" ]]
	then
		DOCKER_IMAGE_TAGS+=("${LIFERAY_DOCKER_REPOSITORY}jinjimosd/${DOCKER_IMAGE_NAME}:latest")
	fi
